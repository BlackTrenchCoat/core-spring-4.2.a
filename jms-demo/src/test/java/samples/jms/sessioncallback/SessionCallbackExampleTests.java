...
package samples.jms.sessioncallback;
...
public class SessionCallbackExampleTests extends AbstractMessagingTests {
	...
	public void testSessionCallback() {
		...
		jmsTemplate.execute(new SessionCallback<Object>() {
			public Object doInJms(Session session) throws JMSException {
				Queue queue = session.createQueue("logging.queue");
				MessageProducer producer = session.createProducer(queue);
				Message message = session.createTextMessage("Hello Queue!");
				message.setJMSCorrelationID("Hello-1");
